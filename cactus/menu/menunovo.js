...
function mopen(id) {	
	...
	if(ddmenuitem) {
		...
		document.getElementById(id_ativo+'_menu').style.borderColor='transparent';
		// MUDA O ESTILO DO MENU PARA DESATIVADO
	}
	
	document.getElementById(id+'_menu').className='menu_ativo';
	document.getElementById(id+'_menu').style.borderColor='#000';
	// MUDA O ESTILO DO MENU PARA ATIVADO

	ddmenuitem = document.getElementById(id);

	if( !document.getElementById('frame_fundo_menu') ) {
		document.body.innerHTML=document.body.innerHTML+"<iframe src='about:blank' frameborder=no id=frame_fundo_menu width=1 height=1 style='display:block; z-index:5; position:absolute; left:1px; top:1px' ></iframe>";
...
function mopen_sub(id) {	
	...
	ddmenuitem_sub.style.left = (ddmenuitem.style.left+ddmenuitem.offsetWidth)+"px";
