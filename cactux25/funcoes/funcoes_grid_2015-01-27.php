...
class grid {
	...
	function abrir_estilo( $width = '99%', $extra = '' ) {
		
		$cmd = "<table width='$width' $extra   >"; //cellpadding=0 cellspacing=0 border=1
	...
	function gerar($cabeca,$valores) {
		...
		if( is_array($cabeca[0]) ) 
			grid::tabela($cabeca[0],$cabeca[4],$cabeca[1],$cabeca[2]);
	...
	function gerar_estilo($cabeca,$valores) {
		...
		grid::abrir_estilo($cabeca[3],$cabeca[4]);
		
		if( is_array($cabeca[0]) ) 
			grid::tabela($cabeca[0],$cabeca[5],$cabeca[1],$cabeca[2]);
	...
	function gerar_id($cabeca,$valores,$id) {
		...
		if( is_array($cabeca[0]) ) 
			tabela($cabeca[0],$cabeca[4],$cabeca[1],$cabeca[2]);
	...
	function ordenar($desc,$ordena='',$extra='',$busca='') {
		...
		$r = "<a class=link_ordena href=# onclick=Ordenar('$ordena')>".$desc."</a>";
	...
	function filtro_busca() {
		...
		foreach( $_POST as $chave => $valor  ) {
			...
			if( (substr($chave,0,6) == 'busca_' || $chave=='n') && $valor ) {
...
//		$CMD .= "<a $target class=paginar2 href=$PHP_SELF?busca=$busca&n=$anterior> <U><< Anterior</U> </a>";
		$CMD .= "<a $target class=paginar2 onclick=paginacao('$anterior') href=#> <U><< Anterior</U> </a>";
	} 
	...
	if( $proximo >= $total_pesq ) { 
		...
		$CMD .= "<a $target class=paginar3 onclick=paginacao('$proximo') href=# > <U>Próximo >></U></a>";
	} 
	$CMD .= "<BR>";
	...
	javascript("function paginacao(valor) {document.form_paginacao.n.value=valor;document.form_paginacao.submit();}");
