...
//function combo_bd($item, $nome, $valor1, $valor2, $tabela, $extra='') {
global $TABULACAO, $AJAX, $EXEMPLOS;
...
/*
if( $extra == '' )
	$extra = "ORDER BY ".$TAB."_tx_nome";

if( $extra == 'modificar' || $_POST[acao] == 'modificar' ) {
	$modificar=1;
}
 */
...
if( $AJAX != 'ativo' ) {
	$comp .= "<select name=$nome $extra   tabindex=".$TABULACAO++."> \r\n";
     if( $valor1 != '' && $valor2 != '' ) {  
        //printf("<option value=%s> %s </option>", $valor1,$valor2); 
		$comp .= "<option value=\"$valor1\"> $valor2 </option>"; 
     } 
} else
...
//$sql_combo = "SELECT * FROM $tabela WHERE ".$TAB."_tx_status='ativo' AND ".$campos ;
//$sql_combo = "SELECT * FROM $tabela WHERE ".$TAB."_tx_status='ativo' ORDER BY ".$TAB."_tx_nome";

//$sql_combo = mysql_query($sql_combo) or die(trata_erro());
...
while( $array_combo = fetch_array($sql_combo) ) {  
	...
	$comp .= "<option value=\"$array_combo[0]\">".$mostra." </option>\r\n"; 
} 
...
/*
function combo_div($item, $nome, $valor1, $valor2, $arquivo, $x=550, $y=350, $extra='') {
global $NOME_FORM, $TABULACAO, $PHP_SELF, $AJAX;

$arq_orig = $arquivo.".php";
...
//$comp .=  "<input type=text readonly name=nome_".$nome." value=\"$valor2\">  \r\n";
$comp .= "<input type=text id=nome_".$nome." name=nome_".$nome." value=\"$valor2\" $extra onkeyup=combo_div('".$nome."','".$form."','".$arquivo."') tabindex=".$TABULACAO++.">  \r\n";
$comp .= "<input type=hidden name=orig_".$nome." value=\"$valor2\" > \r\n";
$comp .= "<a href=#>";

$comp .=  "<img id=img_".$nome." border=0 src=$diretorio/imagens/localizar.gif OnClick=busca_div('".$nome."','".$form."','".$arquivo."')>";
$comp .=  "</a> \r\n";

$comp .= "<div id='$arquivo' style=display:none>";
//$comp .= "<iframe frameborder=no src='".$arq_orig."?frame=$arquivo&campo=$nome&form=$form&t=$TABULACAO' name='".$arquivo."_frame' width=210 height=210 scrolling=no ></iframe>";
$comp .= "<iframe frameborder=no src='".$arq_orig."?frame=$arquivo&campo=$nome&form=$form&t=$TABULACAO' name='".$arquivo."_frame'  width=210 height=220 scrolling=no ></iframe>";
$comp .= "</div>";
