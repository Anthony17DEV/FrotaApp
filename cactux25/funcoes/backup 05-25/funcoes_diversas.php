...
$CMD = "<CENTER>";
 ...
 if( $inicio == 0 ) { 
   $CMD .= "<span class=texto1> << Anterior </span>";
 } else { 
   $CMD .= "<a class=texto1 href=$PHP_SELF?busca=$busca&n=$anterior> <U><< Anterior</U> </a>";
 } 
...
for($i=$INICIO_PAGINAR;$i!=$FIM_PAGINAR;$i++) {
	$VALOR = $MAX_PAG*$i;
	$CMD .= "<a class=texto1 href=$PHP_SELF?busca=$busca&n=$VALOR> ";
...
$CMD .= "&nbsp;";
 ...
 if( $proximo >= $total_pesq ) { 
   $CMD .= "<span class=texto1> Próximo >> </span>";
 } else { 
   $CMD .= "<a class=texto1 href=$PHP_SELF?busca=$busca&n=$proximo > <U>Próximo >></U></a>";
 } 
$CMD .= "<BR>";
$CMD .= "<span class=texto1> <B>$atual de $total_pag</b>";
$CMD .= "<BR><BR>";
