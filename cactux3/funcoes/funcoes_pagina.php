...
function seleciona_combo_net($id,$nome) {
	...
	$comp = "<a href=\"javascript:seleciona_combo_net($id,'$nome_js')\">$nome</a>";
...
function set_titulo_janela($titulo='') {
	...
	javascript("janela_titulo('$titulo')");
...
function acao_botao($nome,$acao='',$campos='',$valores='',$action='',$metodo='',$extra='') {
	...
	cactux::acao_botao($nome,$acao,$campos,$valores,$action,$metodo,$extra);
...
function icone_modificar($id, $acao, $campos='', $valores='', $action='', $extra='', $alt='',$img="cactus/imagens/visualizar.png") {
	global $CACTUX_CONF;
	
	if( $CACTUX_CONF['path'] != '' ) {
		$img = $CACTUX_CONF['path']."//".$img;
	} // se o path for diferente de '', ou seja, se estiver fora da raiz, entao usa o path
...
// RESOLVE O PROBLEMA COM OS ICONES
$img = str_replace("//","/",$img);

$CAMPOS = explode(",", $campos);
$VALORES = explode(",",$valores);

$nome_form = "m".rand(0,999);
cria_form($nome_form.$id,"acao,id,".$campos,"modificar,$id,".$valores,$acao,'',$extra);

//	$rand = rand(0,9999);
//	$comp .= "<div id=div_ajuda_$rand style='display:none; position:absolute; background:yellow; font-family:arial; font-size:10px; padding: 2px 2px 2px 2px'><b>$alt</div>";
	
//	$comp .= "<img src='$src' border=$border $tam $larg $alt $extra onmouseover=exibe_ajuda($rand,this) onmouseout=remove_ajuda($rand) >";



$comp .= "\r\n <a href=\"javascript:document.".$nome_form.$id.".submit()\" title='$alt'>";
...
function icone_excluir($id, $acao, $campos='', $valores='', $action='', $extra='', $alt='',$img="cactus/imagens/excluir.png") {
	global $CACTUX_CONF;
...
function botao_excluir($id, $campos='', $valores='', $nome='', $acao='', $act='',$extra='', $img) {
global $PHP_SELF; // para pegar a variavel do arquivo que a chama!
...
$nome_form = "e".rand(0,999);

cria_form($nome_form.$id,"acao,$nome,".$campos,"$acao,$id,".$valores,$act,'',$extra);

//$comp .= "\r\n <a href=\"javascript:excluir('$nome_form$id')\">";
$comp = "\r\n <a href=\"javascript:excluir('$nome_form$id')\">";
//$comp .= "\r\n <a href=javascript:void(0)>";
exibir_componente($comp);
imagem("$img",'','','','Excluir',"");
