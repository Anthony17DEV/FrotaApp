...
if( $_POST[cactux_tipo_form] == 'mobile' ) {
		...
		// ADICIONA O TIPO TEL PARA APARECER O TECLADO NUMERICO NO CELULAR
		
		$MASCARA_VALOR = " type='tel' maxlength=14 onkeyup='mascara_reais(this);' ";
		$MASCARA_CPF = " type='tel' maxlength=14  onkeyup='mascara_cpf(this);' ";
		$MASCARA_CNPJ = " type='tel' maxlength=18  onkeyup='mascara_cnpj(this);' ";
		$MASCARA_CEP = " type='tel' maxlength=9  onkeyup='mascara_cep(this);' ";
		$MASCARA_FONE = " type='tel' maxlength=15  onkeyup='mascara_telefone(this);' ";
		$MASCARA_DATA = "  type='tel' maxlength=10  onkeyup='mascara_data(this);' ";
		$MASCARA_PLACA = " maxlength=8  onkeyup='mascara_placa(this);' ";
		$MASCARA_NUMERO = " type='tel' maxlength=12 onkeyup='mascara_numeros(this);' ";
		$MASCARA_HORA = " type='tel' maxlength=5  onkeyup='mascara_hora(this);' ";

} else {

		$MASCARA_VALOR = " maxlength=14 onkeyup='mascara_reais(this);' ";
		$MASCARA_CPF = " maxlength=14  onkeyup='mascara_cpf(this);' ";
		$MASCARA_CNPJ = " maxlength=18  onkeyup='mascara_cnpj(this);' ";
		$MASCARA_CEP = " maxlength=9  onkeyup='mascara_cep(this);' ";
		$MASCARA_FONE = " maxlength=15  onkeyup='mascara_telefone(this);' ";
		$MASCARA_DATA = "  maxlength=10  onkeyup='mascara_data(this);' ";
		$MASCARA_PLACA = " maxlength=8  onkeyup='mascara_placa(this);' ";
		$MASCARA_NUMERO = " maxlength=12 onkeyup='mascara_numeros(this);' ";
		$MASCARA_HORA = " maxlength=5  onkeyup='mascara_hora(this);' ";
}
...
define('MASCARA_HORA',$MASCARA_HORA);


class cactux_cookie {

	
	function apaga_cookies_login($sistemacactus) {
		
//		$fp = fopen("/tmp/log_cookie.txt","a") or die("erro");
//		fputs($fp,date("H:i:s")." -> apaga_cookies_login\n");
//		echo "!!!!!!!!!!!!!!!!";
		
		$nome_cookie = str_replace(" ","",$sistemacactus);
		$past = time() - 3600;
		foreach ( $_COOKIE as $key => $value ){
			
			$linha .= "    if( ".substr($key,0,strlen($nome_cookie)+1)." == ".$nome_cookie."_ ) -> ";
			
			// APAGA APENAS OS COOKIES QUE EXISTIREM DESSE SISTEMA
			if( substr($key,0,strlen($nome_cookie)+1) == $nome_cookie."_" ) {
...
//		fclose($fp);
	...
	function prefixo_cookie($nome_sistema) {
	
//		$nome_cookie = str_replace(array(" ","&","?",".",",","%","$","#","'","`"),array(""),$nome_sistema);
		$nome_cookie = preg_replace('/[^-A-Z0-9_]+/', '', strtoupper($nome_sistema));
	...
	/*** CACTUS **************************************************************\
	|
	| SE RECEBE O NOME DO ARQUIVO, VERIFICA SE EH PRA ADCICIONAR ? OU & AO URL
	| SE NAO RECEBE, RETORNA APENAS A CHAVE=VALOR E VOCE RESOLVE MANUALMENTE
	|
	\*************************************************************************/
	function url_sistema($arquivo='') {
		global $CONFIGURA;

		$linha = "sistemacactus=".cactux_cookie::prefixo_cookie($CONFIGURA[NOME_SISTEMA]);
	...
	function hidden_sistema() {
		global $CONFIGURA;
		
		$linha = "<input type=hidden name=sistemacactus value='".cactux_cookie::prefixo_cookie($CONFIGURA[NOME_SISTEMA])."' />";
...
// TRAS AS VARIAVEIS DE _POST[busca_
...
// TRAS AS VARIAVEIS DE COOKIE DO SISTEMA
cactux_cookie::hidden_sistema();

echo "</form> \r\n";
echo "<script>";
//echo "	window.document.onload=$nome.submit();";
echo " window.document.onload=window.document.forms[0].submit();";
echo "</script>\r\n";
echo "</body>";
...
$ext = pathinfo($_FILES[$NOME]['name'], PATHINFO_EXTENSION);
// echo "!$ext!";

// $arquivo['name'] = $arq.substr($arquivo['name'],-4 );
$arquivo['name'] = $arq.".".$ext;
...
case 0:
	if( $data == '30' ) {
		...
		switch( $data ) {
		case 'hoje':	$data = date("d/m/Y");			break;
		case 'ontem':	$data = date("d/m/Y",mktime(0,0,0,date("m"), date("d")-1, date("Y")));	break;
		}
...
case 3: 
 // se for pra transformar no formato do MySQL ano-mes-dia hora-minuto-segundo
		...
		switch( $data ) {
		case 'hoje':	$data = date("d/m/Y H:i:s");			break;
		case 'ontem':	$data = date("d/m/Y H:i:s",mktime(0,0,0,date("m"), date("d")-1, date("Y")));	break;
		}
